"""! @brief Definition of the MXContainer class."""
...
## MX type: https://web.casadi.org/docs/#document-symbolic
MX = cs.casadi.MX
...
class MXContainer(collections.OrderedDict):
    """! Container for MX variables"""

    ## Dict[str, bool]: labels for each item in dict, true means variables are discrete
    is_discrete = {}

    def __add__(self, other):
        """! Add two MXContainer's.

        @param other Another MXContainer instance.
        @return An MXContainer instance containing elements from both this and the other instance.
        """
        assert isinstance(
            other, MXContainer
        ), f"cannot add MXContainer with a variable of type {type(other)}"
        out = MXContainer()
    ...
    def __setitem__(self, label: str, value: MX) -> None:
        """! Set new MX item.
        ...
        assert isinstance(
            value, (MX, float)
        ), f"value must be of type casadi.casadi.MX/float, not {type(value)}"
        if label in self:
            raise KeyError(f"'{label}' already exists")
        super().__setitem__(label, cs.MX(value))
        self.is_discrete[
            label
    ...
    def variable_is_discrete(self, label: str) -> None:
        ...
        @param label Name for a specific item that appears in the MXContainer instance.
        """
    ...
    def has_discrete_variables(self) -> bool:
        ...
        @return Boolean indicating if the MXContainer instance contains discrete variables.
        """
    ...
    def numel(self) -> int:
        ...
        @return Number of elements in the MXContainer instance.
        """
        return self.vec().numel()

    def vec2dict(self, vec: Union[DM, np.ndarray, list, tuple]) -> dict:
        """! Convert vector with same layout as MXContainer to dict.

        @param vec Vector assumed to be arranged with same layout as MXContainer instance.
    ...
    def dict2vec(self, d: Dict[str, ArrayType]) -> Union[DM, MX]:
